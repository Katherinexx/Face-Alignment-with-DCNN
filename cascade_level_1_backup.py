...
def model(data,train=False):
	...
	relu_shape = activation.get_shape().as_list()
...
# opt = tf.train.GradientDescentOptimizer(learning_rate=LEARNING_RATE).minimize(regularized_train_loss,global_step=batch)
...
with tf.Session() as sess:
	...
	for step in xrange (int(NUM_EPOCHS * train_size) // BATCH_SIZE):
		...
		_, l, to = sess.run([opt, train_loss, train_output],
						feed_dict=feed_dict)
		...
		if step % EVAL_FREQUENCY == 0:
			...
			sys.stdout.flush()
	print('Validation loss: %.5f' % eval_in_batches(test_images,test_labels,sess))
	saver.save(sess,"./model/sigmoid_50epoch.ckpt")
	print ('Model saved')
