...
train_images, train_labels, mean_shape = utils.get_average_shape_centered_data()
...
mean_shapes = np.zeros((test_size,2*N_POINTS))
for i in xrange(test_size):
	mean_shapes[i,:] = mean_shape
test_prediction = tf.add(model(test_images_node),tf.constant(mean_shapes, dtype=LABEL_DATA_TYPE))
...
with tf.Session() as sess:
	writer = tf.train.SummaryWriter("./logs/mean_shape_centered_logs",sess.graph)
	...
	saver.save(sess,"./model/mean_shape_centered.ckpt")
	print ('Model saved')
